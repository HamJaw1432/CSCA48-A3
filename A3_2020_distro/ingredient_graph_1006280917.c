...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
    ...
    * Implement this function
    * ********/
	int i_of_ingredient = ingredient_index(source_ingredient);
	if(i_of_ingredient != -1)
	{
		int j = 0;
		while(j < MAT_SIZE)
		{
			if(AdjMat[i_of_ingredient][j] > 0)
			{
				printf("%s\n", ingredients[j]);
			}
			j++;
    ...
    *******/
	...
	if(k >= 0)
	{
		...
		if (i_of_ingredient != -1)
		{
			while(j < MAT_SIZE)
			{
				//printf("%s -- %d----%f\n", ingredients[j], j, AdjMat[i_of_ingredient][j]);
				if(AdjMat[i_of_ingredient][j] > 0)
				{
					if(searchInt(h, j) == 0)
					{
					...
					h = related_k_dist(h, ingredients[j], k - 1, dist + 1);
				}
				j++;
			}
		}
...
intNode *related_with_restrictions_k(intNode *h, intNode *p, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
		...
		if (i_of_ingredient != -1)
		{
			...
			while(j < MAT_SIZE)
			{
				...
				if(AdjMat[i_of_ingredient][j] > 0 )
				{
					...
					h = related_with_restrictions_k(h, p, ingredients[j], k - 1, dist + 1);
...
void related_k_array(int avoid_a[MAT_SIZE], char source_ingredient[MAX_STR_LEN], int k, int dist)
{
	...
	if(k >= 0)
	{
		...
		if (i_of_ingredient != -1)
		{
			while(j < MAT_SIZE)
			{
				...
				if(AdjMat[i_of_ingredient][j] > 0)
				{
					avoid_a[j] = avoid_a[j] + 1;
					related_k_array(avoid_a, ingredients[j], k - 1, dist + 1);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
	...
	/* intNode *avoid_h = NULL;
	avoid_h = related_k_dist(avoid_h, avoid, k_avoid, 0);
	int i_of_ingredient_to_avoid = ingredient_index(avoid);
	avoid_h = insertInt(avoid_h, i_of_ingredient_to_avoid);
	
	
	intNode *h = NULL;
	h = related_with_restrictions_k(h, avoid_h, source_ingredient, k_source, 0);
	
	deleteList(avoid_h);
	return h; */
	
	int avoid_a[MAT_SIZE];
	int keep_a[MAT_SIZE];
	for(int q = 0; q < MAT_SIZE; q++)
	{
		avoid_a[q] = 0;
		keep_a[q] = 0;
	}
	int i_of_ingredient_to_avoid = ingredient_index(avoid);
	avoid_a[i_of_ingredient_to_avoid] = 1;
	
	related_k_array(avoid_a,avoid,k_avoid,0);
	
	related_k_array(keep_a,source_ingredient,k_source,0);	
	
	intNode *h = NULL;
	for(int w = 0; w < MAT_SIZE; w++)
	{
		if(avoid_a[w] == 0 && keep_a[w] != 0)
		{
			h = insertInt(h, w);
		}
	} 
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    ******/
	...
	int j = 0;
	int i_of_ingredient;
	int i_to_change_in_recipe = -1;
	
	while(j < 10)
	{
		if(strcmp(to_change, recipe[j]) != 0 && strcmp(recipe[j],"") != 0)
		{
			i_of_ingredient = ingredient_index(recipe[j]);
			int k = 0;
			while(k < MAT_SIZE)
			{
				if(AdjMat[i_of_ingredient][k] > 0)
				{
					temp[k] = temp[k] + AdjMat[i_of_ingredient][k];
				}
				k++;
			}
		}
		else if (strcmp(to_change, recipe[j]) == 0)
		{
			i_to_change_in_recipe = j;
		}
		j++;
	}
	double highest_sc = -1;
	...
	for(int i = 0; i < MAT_SIZE; i++)
	{
		if(temp[i] > highest_sc)
		{
			int z = 0;
			int found = 0;
			while(z < 10)
			{
				if(strcmp(ingredients[i],recipe[z]) == 0)
				{
					found = 1;
				}
				z++;
	...
	if(i_to_change_in_recipe != -1)
	{
		if(highest_i != -1)
		{
			strcpy(recipe[i_to_change_in_recipe], ingredients[highest_i]);
		}
		else
		{
			strcpy(recipe[i_to_change_in_recipe],"");
		}
	}	
}
